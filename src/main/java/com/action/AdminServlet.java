...
import main.java.com.bean.AdminBean;
import main.java.com.bean.SystemBean;
		...
		try{
			...
			if(method.equals("one")){//admin登录
				...
				if(username == null||username.trim().equals("")){
					...
					int flag = loginbean.adminLogin(username,password);
					switch (flag){
				...
				if(username2 == null){
					...
					if(ra.equals("add")){
						...
						if(flag == Constant.SUCCESS){
							...
							request.setAttribute("message", "请确认此管理员是否存在！");
							request.getRequestDispatcher(sysdir+"/system/user.jsp").forward(request, response);
						}		
					}
				}
